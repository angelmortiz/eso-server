...
/** APIS */
export const apiGetExercises = catchAsync(
...
export const apiGetExerciseNames = catchAsync(
  async (req: Request, res: Response) => {
    const exerciseNames = await ExerciseHandler.fetchAllNames();
    res
      .status(RESPONSE_CODE.OK)
      .json(RESPONSE.FETCHED_SUCCESSFULLY(exerciseNames));
  }
);

export const apiGetExerciseById = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!exercise) {
      return next(
        new AppError(`No exercise found using id '${exerciseId}'.`, 404)
      );
    }
...
export const apiAddExercise = catchAsync(
  async (req: Request, res: Response) => {
    await ExerciseHandler.save(req.body);
    res.status(RESPONSE_CODE.CREATED).json(RESPONSE.ADDED_SUCCESSFULLY());
  }
);

export const apiUpdateExercise = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const exerciseId: string = req.params.exerciseId;
    if (!exerciseId) {
      return next(
        new AppError(`No exerciseId found in the parameters.`, 404)
      );
    }

    await ExerciseHandler.update(exerciseId, req.body);
    res.status(RESPONSE_CODE.CREATED).json(RESPONSE.UPDATED_SUCCESSFULLY());
  }
);

export const apiDeleteExercise = catchAsync(
  async (req: Request, res: Response) => {
    const exerciseId: string = req.params.exerciseId;
