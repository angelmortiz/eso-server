...
/** APIS */
export const apiGetPrograms = catchAsync(
...
export const apiGetProgramNames = catchAsync(
  async (req: Request, res: Response) => {
    const programNames = await ProgramHandler.fetchAllNames();
    res
      .status(RESPONSE_CODE.OK)
      .json(RESPONSE.FETCHED_SUCCESSFULLY(programNames));
  }
);

export const apiGetProgramById = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!program) {
      return next(
        new AppError(`No program found using id '${programId}'.`, 404)
      );
    }
...
export const apiAddProgram = catchAsync(async (req: Request, res: Response) => {
  await ProgramHandler.save(req.body);
  res.status(RESPONSE_CODE.CREATED).json(RESPONSE.ADDED_SUCCESSFULLY());
});

export const apiUpdateProgram = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const programId: string = req.params.programId;
    if (!programId) {
      return next(new AppError(`No programId found in the parameters.`, 404));
    }
    await ProgramHandler.update(programId, req.body);
    res.status(RESPONSE_CODE.CREATED).json(RESPONSE.UPDATED_SUCCESSFULLY());
  }
);

export const apiDeleteProgram = catchAsync(
  async (req: Request, res: Response) => {
    const programId: string = req.params.programId;
