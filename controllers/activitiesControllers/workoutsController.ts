...
/** APIS */
export const apiGetWorkouts = catchAsync(
...
export const apiGetWorkoutNames = catchAsync(
  async (req: Request, res: Response) => {
    const workoutNames = await WorkoutHandler.fetchAllNames();
    res
      .status(RESPONSE_CODE.OK)
      .json(RESPONSE.FETCHED_SUCCESSFULLY(workoutNames));
  }
);

export const apiGetWorkoutById = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!workout) {
      return next(
        new AppError(`No workout found using id '${workoutId}'.`, 404)
      );
    }
...
export const apiAddWorkout = catchAsync(async (req: Request, res: Response) => {
  await WorkoutHandler.save(req.body);
  res.status(RESPONSE_CODE.CREATED).json(RESPONSE.ADDED_SUCCESSFULLY());
});

export const apiUpdateWorkout = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const workoutId: string = req.params.workoutId;
    if (!workoutId) {
      return next(new AppError(`No workoutId found in the parameters.`, 404));
    }

    await WorkoutHandler.update(workoutId, req.body);
    res.status(RESPONSE_CODE.CREATED).json(RESPONSE.UPDATED_SUCCESSFULLY());
  }
);

export const apiDeleteWorkout = catchAsync(
  async (req: Request, res: Response) => {
    const workoutId: string = req.params.workoutId;
